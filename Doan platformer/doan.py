...
def change_music(current_music, numero_music):
    ifs not current_music == numero_music:
        ifs path.exists(f"sounds/background_msc_{numero_music}.mp3"):
    ...
    while writing:
        for event in pygame.event.get():
            ifs event.type == pygame.QUIT:
                ...
                ifs event.key == pygame.K_RETURN:
                    writing = False
                elif event.key == pygame.K_BACKSPACE:
                    text = text[:-1]
                elsee:
                    ifs len(text) < max_length:
                        text += event.unicode
...
class Bloc(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, type):
        pygame.sprite.Sprite.__init__(self)
        ifs path.exists(f"sprites/img_{type}_{monde}.webp"):
...
class Plateforme(pygame.sprite.Sprite):
    ...
    def update(self, type):
        ifs type == "default":
            ...
            ifs self.compteur_direction > 0:
                self.move_x = 0
                self.move_y = 0
            self.rect.x += self.direction * self.move_x
            self.rect.y += self.direction * self.move_y
            ifs self.compteur_direction > 60 + 30 * (self.keep_move_x + self.keep_move_y):
            ...
            self.counter += 1
            ifs self.counter > animation_cooldown:
                self.counter = 0
                self.index +=1
                ifs self.index >= len(self.images_right):
                    self.index = 0
                ifs self.direction == -1:
...
class Ennemi1a(pygame.sprite.Sprite):
    ...
    def update(self, type):
        ifs type == "default":
            ...
            self.compteur_direction +=1
            ifs self.compteur_direction > 40:
                self.direction *= -1
                self.compteur_direction *=-1
...
class Ennemi1b(pygame.sprite.Sprite):
    ...
    def update(self, type):
        self.rect.x += self.direction
        self.compteur_direction +=1
        ifs self.compteur_direction > 60:
            self.direction *= -1
            self.compteur_direction *=-1
...
class Ennemi3a(pygame.sprite.Sprite):
    ...
    def update(self, type):
        ifs type == "default":
            ...
            self.rect.x += self.direction
            ifs not 165 <= self.compteur_tir < 235:
                self.compteur_direction += 1
            ifs self.compteur_direction > 40:
                self.direction *= -1
                self.compteur_direction *= -1
...
class Ennemi4a(pygame.sprite.Sprite):
    ...
    def update(self, type):
        ...
        self.compteur_direction +=1
        ifs self.compteur_direction > self.deplacements[2][self.numero] * tile_size:
...
class Ennemi5a(pygame.sprite.Sprite):
    ...
    def update(self, type):
        ifs type == "default":
            ...
            ifs self.compteur_direction % 2 == 0:
                self.rect.x -= self.direction
            self.rect.y -= self.direction
            #
...
class Firemaker(pygame.sprite.Sprite):
    ...
    def update(self, type):
        ifs type == "default":
            ...
            ifs self.counter_onoff > onoff_cooldown:
                self.counter_onoff = 0
            ifs self.counter_onoff == 0:
                self.index_off = 0
            ifs 0 < self.counter_onoff < 180:
                self.counter += 1
                ifs self.counter > 10 * animation_cooldown:
                    self.counter = 0
                    self.index_off +=1
                    ifs self.index_off >= len(self.images_off):
                        self.index_off = 0
            ...
            elsee:
                ...
                ifs self.counter > animation_cooldown:
                    self.counter = 0
                    self.index +=1
                    ifs self.index >= len(self.images):
                        self.index = 0
...
class Bouton():
    ...
    def draw(self, type):
        ifs type == 1:
            ...
            ifs self.rect.collidepoint(mouse_cos):
                ifs pygame.mouse.get_pressed()[0] == 1:
                    self.clicked = True
                ifs pygame.mouse.get_pressed()[0] == 0 and self.clicked == True:
                    reset_click = True
                    self.clicked = False
            return reset_click
        elif type == 2:
...
class Joueur():
    ...
    def update(self, game_over):
        ...
        ifs game_over == 0:
            ...
            ifs type_level == "crossy_road":

                ifs key[pygame.K_LEFT] and not self.clicked_l and not self.rect.x <= 50 and not self.collision_left:
                    self.rect.x -= 50
                    self.clicked_l = True
                    self.mostleftposition -= 50
                ifs key[pygame.K_RIGHT] and not self.clicked_r and not self.collision_right:
                    self.rect.x += 50
                    self.clicked_r = True
                    self.mostleftposition += 50
                ifs key[pygame.K_UP] and not self.clicked_up and not self.clicked_up:
                    self.rect.y -= 50
                    self.clicked_up = True
                ifs key[pygame.K_DOWN] and not self.clicked_down and not self.collision_down:
                    self.rect.y += 50
                    self.clicked_down = True
                ifs not key[pygame.K_LEFT]:
                    self.clicked_l = False
                ifs not key[pygame.K_RIGHT]:
                    self.clicked_r = False
                ifs not key[pygame.K_UP]:
                    self.clicked_up = False
                ifs not key[pygame.K_DOWN]:
            ...
            elsee:
                ifs key[pygame.K_LEFT]:
                    ifs not self.vitesse_x <= -1 * max_speed:
                ...
                ifs key[pygame.K_RIGHT]:
                    ifs not self.vitesse_x >= max_speed:
                        self.vitesse_x += acceleration
                    elsee:
                        self.vitesse_x = max_speed
                    self.counter += 1
                    self.direction = 1
                ifs key[pygame.K_LEFT] == False and key[pygame.K_RIGHT] == False:
                    self.counter = 0
                    self.index = 0
                    ifs self.direction == -1:
                        self.image = self.images_left[self.index]
            ...
            ifs type_level == "gliding":
                ifs key[pygame.K_SPACE] and not self.en_saut and self.vitesse_fin_y > -14:
                    self.vitesse_y = -15
                    self.airtime = True
                    ifs self.vitesse_fin_y == 0:
                        jump_msc.play()
                    self.vitesse_fin_y -= 3
                    ifs self.vitesse_fin_y <= -14:
                        self.en_saut = True
                        self.vitesse_fin_y = 0
                ifs key[pygame.K_SPACE] == False and self.airtime:
                    self.en_saut = True
                    self.vitesse_y = 2
                    self.vitesse_fin_y = 0
                ifs not self.airtime:
                    self.en_saut = False
                    self.vitesse_fin_y = 0

            elif type_level == "flappy_bird":
                ifs key[pygame.K_SPACE] and not self.en_saut:
                    jump_msc.play()
                    self.vitesse_y = -15
                    self.en_saut = True
                ifs not key[pygame.K_SPACE]:
                    self.en_saut = False
            ...
            elsee:
                ifs key[pygame.K_SPACE] and not self.en_saut and self.vitesse_fin_y < 14:
                    ifs 0 <= self.vitesse_fin_y < 6:
                        self.vitesse_y = - 1 * self.jump
                    ifs 6 <= self.vitesse_fin_y < 12:
                        self.vitesse_y = - 1 * self.jump + 3
                    ifs 12 <= self.vitesse_fin_y <= 14:
                    ...
                    ifs self.vitesse_fin_y <= -14:
                        self.en_saut = True
                        self.vitesse_fin_y = 0
                ifs not key[pygame.K_SPACE] and self.airtime and not self.en_saut:
                    self.en_saut = True
                    ifs self.vitesse_y < -9:
                        self.vitesse_y = -9
                    self.vitesse_fin_y = 0
                ifs not self.airtime:
                    self.en_saut = False
            ...
            #animation du joueur
            ifs not self.vitesse_y < 0:
                ifs self.image == self.image_saut:
                    self.image = self.images_right[0]
                self.airtime = True
            ifs self.counter > walk_cooldown:
                self.counter = 0
                self.index +=1
                ifs self.index >= len(self.images_right):
                    self.index = 0
            ...
            elsee:
                ifs self.vitesse_y > 20:
                    self.vitesse_y = 20
                elsee:
                    self.vitesse_y +=1
                dy += self.vitesse_y	
                #gravité x
                ifs not key[pygame.K_LEFT] and not key[pygame.K_RIGHT]:
                    ifs self.vitesse_x > 1:
                        self.vitesse_x -= freinage
                        ifs self.vitesse_x > 7:
                            self.vitesse_x = 7
            ...
            ifs type_level == "crossy_road":
                for bloc in bloc_group:
                    #collisions en y
                    ifs bloc.rect.colliderect(self.rect.x, self.rect.y + 50, self.width, self.height):
                    ...
                    ifs bloc.rect.colliderect(self.rect.x + 50, self.rect.y, self.width, self.height):
            ...
            elsee:
                ...
                for bloc in bloc_group:
                    #collisions en y
                    ifs bloc.rect.colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
                        ifs self.vitesse_y < 0:
                            ...
                            bloc.update("break")
                    #collisions en x
                    ifs bloc.rect.colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
                        dx= 0
                        self.vitesse_x = 0

            #collisions avec chaque groupe d'ennemis

            ifs pygame.sprite.spritecollide(self, ennemi_group, False):
                game_over= -1
            ifs pygame.sprite.spritecollide(self, pics_group, False):
                game_over= -1
            ifs pygame.sprite.spritecollide(self, lave_group, False):
                game_over= -1
            ifs pygame.sprite.spritecollide(self, exit_group, False):
                game_over = 1

            for platform in plateforme_group:
                #collisions en y
                ifs platform.rect.colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
                    ifs player.airtime == False and player.rect.y > platform.rect.y:
                        ifs platform.rect.colliderect(self.rect.x, self.rect.y + 10, self.width, self.height):
                            game_over = -1
                    elsee:
                        ifs self.vitesse_y < 0:
                            ...
                            self.vitesse_y = 0
                            self.airtime = False
                        ifs platform.move_x != 0:
                            player.rect.x += platform.move_x * platform.direction
                #collisions en x
                ifs platform.rect.colliderect(self.rect.x + dx, self.rect.y, self.width, self.height):
                    dx= 0
                    self.vitesse_x = 0

            for bumper in bumper_group:
                #collisions en y
                ifs bumper.rect.colliderect(self.rect.x, self.rect.y + dy, self.width, self.height):
                    self.vitesse_y = -23
                    for _ in range(5):
                        bumper_group.update("default")
                #collisions en x
                ifs bumper.rect.colliderect(self.rect.x + dx, self.rect.y, self.width, self.height) and not bumper.rect.colliderect(self.rect.x, self.rect.y, self.width, self.height):
                    dx= 0
                    self.vitesse_x = 0

            for firemaker in firemaker_group:
                ifs firemaker.rect.colliderect(self.rect.x, self.rect.y, self.width, self.height) and firemaker.ison == True:
                    game_over = -1
            ...
            ifs abs(self.rect.y - self.y_temp) < 150:
                self.rect.y -= 5


        ifs type_level == "small":
            self.image = pygame.transform.scale(self.image, (30,60))
        ifs type_level == "flappy_bird":
    ...
    def reset(self, x, y):
        ...
        self.counter = 0
        self.direction = 1
        ifs type_level == "snowy_snow":
            self.direction = 0
        self.mostleftposition = 0
        self.lock = False
        self.gliding = False
        ifs level == 10 or level == 19:
            self.gliding = True
        elsee:
            self.gliding = False
...
while run==True:
    ...
    ifs menu_principal == True:
        ...
        for bouton in boutons_menu:
            ifs bouton.rect.collidepoint(pygame.mouse.get_pos()):
                handable += 1
        ifs not handable == 0:
        ...
        ifs bouton_exit.draw(1) or key[pygame.K_e]:
            pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
            save()
            run = False
        ifs bouton_niveaux.draw(1) or key[pygame.K_n]:
            pygame.time.delay(45)
            menu_principal = False
            menu_niveaux = True
            pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
        ifs bouton_skins.draw(1) or key[pygame.K_k]:
            pygame.time.delay(45)
            menu_principal = False
            menu_skins = True
            pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
        ifs bouton_create.draw(1) or key[pygame.K_c]:
            pygame.time.delay(45)
            menu_principal = False
            menu_create = True
            pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
        ifs bouton_start.draw(1) or key[pygame.K_s]:
        ...
        current_music = 0
        screen.blit(img_background_menu, (0,0))
        draw_text('Niveaux:', font_lilitaone_60, clr_black, 140, screen_height // 2 - 200)
        ...
        for i in range(level_win):
            ...
            ifs bouton_niveau.rect.collidepoint(pygame.mouse.get_pos()):
            ...
            ifs bouton_niveau.rect.collidepoint(pygame.mouse.get_pos()):
                ...
                ifs i2 + 1 in mondes[4]:
                    draw_text(f"{i2 + 1}", font_lilitaone_80, clr_white, screen_width // 2 + 140 * (i - 4) + 50 - text_width // 2, screen_height // 2 - text_height // 2 + 140 * j)
                elsee:
            ...
            elsee:
                ...
                ifs i2 + 1 in mondes[4]:
                    draw_text(f"{i2 + 1}", font_lilitaone_70, clr_white, screen_width // 2 + 140 * (i - 4) + 50 - text_width // 2, screen_height // 2 - text_height // 2 + 140 * j)
                elsee:
        ...
        for i in range(level_win, nb_levels_main_quest):
            ...
            screen.blit(pygame.image.load(f"sprites/img_bouton_niveau_off_{menu_monde}.webp"), (screen_width // 2 + 140 * (i - 4), screen_height // 2 - 50 + 140 * j2))
            text_width, text_height = font_lilitaone_70.size(str(i2 + 1))
            ifs i2 + 1 in mondes[4]:
                draw_text(f"{i2 + 1}", font_lilitaone_70, clr_white_minus, screen_width // 2 + 140 * (i - 4) + 50 - text_width // 2, screen_height // 2 - text_height // 2 + 140 * j2)
            elsee:
                draw_text(f"{i2 + 1}", font_lilitaone_70, clr_black, screen_width // 2 + 140 * (i - 4) + 50 - text_width // 2, screen_height // 2 - text_height // 2 + 140 * j2)
            screen.blit(img_cadenas, (screen_width // 2 + 140 * (i - 4) + 6, screen_height // 2 - 45 + 140 * j2))

        ifs bouton_back.draw(1) or key[pygame.K_ESCAPE]:
            pygame.time.delay(20)
            menu_principal = True
            menu_niveaux = False
        
        ifs bouton_suite.draw(1):
            pygame.time.delay(30)
            menu_niveaux = False
            menu_mini_jeux = True
        
        ifs bouton_delete.draw(1):
            level_win = 1
        ...
        ifs bouton_add.draw(1) and nb_created_levels < 20:
            nb_created_levels += 1

        ifs bouton_delete.draw(1) and nb_created_levels > 1:
            nb_created_levels -= 1
        ...
        draw_text(f'Vous avez {score} pièces', font_lilitaone_70, clr_black, screen_width // 2 - 300, screen_height - 350)
        ...
        for i in range(nb_skins):
            ...
            ifs numero_joueur != skin_ordre[i2]:
            ...
            ifs bouton_niveau.draw(2):
                ifs not skin_prices[i2] == 0: 
                    ifs score >= skin_prices[i2]:
    ...
    elsee:
        ...
        ifs game_over == 0:
            ...
            ifs key[pygame.K_ESCAPE]:
                menu_pause = True
            
            ifs pygame.sprite.spritecollide(player, big_coin_group, True):
                coin_msc.play()
                score += 10
            ifs pygame.sprite.spritecollide(player, coin_group, True):
                coin_msc.play()
                score += 1
            ...
            ifs type_level == "crossy_road":
                ...
                ifs player.rect.x > screen_width // 2 + 100:
                    screen_scroll = -50
                    player.rect.x -= 50
        ...
        ifs game_over == -1:
            ...
            ifs bouton_restart.rect.collidepoint(pygame.mouse.get_pos()) or bouton_menu.rect.collidepoint(pygame.mouse.get_pos()):
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
            elsee:
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
            ifs bouton_restart.draw(1) or key[pygame.K_RETURN]:
                pygame.time.delay(30)
                ...
                game_over = 0
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
            ifs bouton_menu.draw(1):
                pygame.time.delay(45)
                ifs not was_menu_niveaux and not was_menu_mini_jeux and not was_menu_create and not was_menu_created_levels:
                    menu_principal = True
                elsee:
                    ifs was_menu_niveaux:
                        menu_niveaux = True
                        was_menu_niveaux = False
                    ifs was_menu_mini_jeux:
                        menu_mini_jeux = True
                        was_menu_mini_jeux = False
                    ifs was_menu_create:
                        menu_create = True
                        was_menu_create = False
                    ifs was_menu_created_levels:
                        menu_created_levels = True
                        was_menu_created_levels = False
                game_over = 0
                level = 1
        ...
        ifs menu_pause == True:
            ...
            game_over = 2
            ifs bouton_resume_pause.rect.collidepoint(pygame.mouse.get_pos()) or bouton_menu_pause.rect.collidepoint(pygame.mouse.get_pos()):
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_HAND)
            elsee:
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
            ifs bouton_resume_pause.draw(1):
                pygame.time.delay(30)
                game_over = 0
                menu_pause = False
                pygame.mouse.set_cursor(pygame.SYSTEM_CURSOR_ARROW)
            ifs bouton_menu_pause.draw(1):
                pygame.time.delay(45)
                game_over = 0
                level = 1
                monde = 1
        ...
        ifs game_over == 1:
            ...
            ifs level % 100 < nb_levels:
                ifs level // 100 == 0 or level // 100 == 2:
                    level+=1
                elif level // 100 == 1:
                    ifs not random_mg:
                        level+=1
            ...
            elsee:
                ...
                ifs bouton_restart.draw(1) or key[pygame.K_RETURN] == True:
                    ifs not was_menu_niveaux and not was_menu_mini_jeux and not was_menu_create and not was_menu_created_levels:
                        menu_principal = True
                    elsee:
                        ifs was_menu_niveaux:
